...
from camper.reg import utils as utl
from camper.sked.models import Event
...
class Sale(models.Model):
    ...
    def send_receipts(self):
        from reg import email_utils as eml
        if self.email:
            try:
                promotional_coupon = CouponCode.objects.get(code='BOGO-{}-{}'.format(self.email, self.id))
            except CouponCode.DoesNotExist:
                promotional_coupon = None
            eml.send_html_email_template(subject='TransparencyCamp Receipt', to_addresses=[self.email], sender="info@transparencycamp.org", template='reg/email_sale.html', context={'sale': self, 'event': self.event, 'coupon': promotional_coupon}, images=[])

        for ticket in self.ticket_set.all():
            if ticket.email and ticket.email != self.email:
                eml.send_html_email_template(subject='TransparencyCamp Receipt', to_addresses=[ticket.email], sender="info@transparencycamp.org", template='reg/email_ticket.html', context={'ticket': ticket, 'event': self.event}, images=[])
